...
export default function AddMap() {
    ...
    async function handleUpload(e:FormEvent<HTMLFormElement>){
        ...
        if(!file) return toast.error("Select map file to upload");
        const checkSum = await computeSHA256(file);
        const id = v4();
        const {data:urlAwsPut,error} = await AWSUpload(file.type,file.size,checkSum,id)
        if(error.length > 0) return toast.error(error);
        const req = await fetch(urlAwsPut, {
            method: "PUT",
            headers: {
        ...
        if(req.ok){
            toast.success("Map Upload Successful to aws");
    ...
    <>
      ...
      <div className={`absolute left-[45%] top-[50%]`}>
        <div className='flex fixed flex-row content-between items-start' >
            <form onSubmit={handleUpload} className="  max-w-sm mx-auto align-middle w-[500px] bg-gray-50  p-[30px] rounded-lg border-2 border-gray-50 ">
                ...
                <div className="flex items-center justify-center w-full my-4">
                    <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                        ...
                        <input onChange={handleFileSelect} id="dropzone-file" name='map' type="file" className="hidden" />
                    </label>
                </div> 
